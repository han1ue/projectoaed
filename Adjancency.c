...
ListNode * InsertAdjacency(int x, int y, int z, char type, ListNode* listhead, Array decoder)
{
    adjacencynode = (Adjacency *) malloc(sizeof(Adjacency) );
        ...
        for(i=0; i<vertices; i++)
        {
            ...
            if ( type == '.')
            {
                if( (x+1 < col) && (matrix[x+1][y][z] != '@') && (matrix[x+1][y][z] != '.'))
                    listhead = InsertAdjacency(x+1, y, z, type, listhead, decoder);
                
                if( (x-1 >= 0) && (matrix[x-1][y][z] != '@') && (matrix[x-1][y][z] != '.'))
                    listhead = InsertAdjacency(x-1, y, z, type, listhead, decoder);
                
                if( (y+1 < row) && (matrix[x][y+1][z] != '@') && (matrix[x][y+1][z] != '.'))
                    listhead = InsertAdjacency(x, y+1, z, type, listhead, decoder);
            ...
            /*If's to find the adjacent vertices for the ramps */
                ...
                listhead = InsertAdjacency(x, y, z-1, type, listhead, decoder);
